...
import importlib
# from models.selecSLS_1 import selecsls60b


def fasterrcnn_selecSLS(pretrained=False, progress=True,
    ...
    backbone = model_module.Net(nClasses=1000, config='SelecSLS60_B')
    backbone.load_state_dict(torch.load('SelecSLS60_B_statedict.pth', map_location=lambda storage, loc: storage))
    model = torchvision.models.detection.FasterRCNN(backbone, num_classes, **kwargs)
    if pretrained:
        state_dict = torch.hub.load_state_dict_from_url('https://download.pytorch.org/models/fasterrcnn_mobilenet_v3_large_320_fpn-907ea3f9.pth', progress=True)
...
def main():
    ...
    detector_model = fasterrcnn_selecSLS(pretrained=True)
    ...
    for nFrame,filename in enumerate(rgb_data_dir.iterdir()):
        ...
        # FASTER RCNN
        model_input = [detector_transform(original_img).to(device)]
