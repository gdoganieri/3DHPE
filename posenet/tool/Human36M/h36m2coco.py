...
subaction_idx = (1, 2)
camera_idx = (1, 2, 3, 4)
action_name = ['Directions', 'Discussion', 'Eating', 'Greeting', 'Phoning', 'Posing', 'Purchases', 'Sitting',
    ...
    for aid in tqdm(action_idx):
        for said in tqdm(subaction_idx):
            for cid in tqdm(camera_idx):
                ...
                joint_world, R, t, f, c, img_widths, img_heights = load_h36m_annot_file(
                    osp.join(root_dir, folder, 'h36m_meta.mat'))
                ...
                for n in range(img_num):
                    ...
                    joint_vis = (joint_img[:, 0] >= 0) * (joint_img[:, 0] < img_widths[n]) * (joint_img[:, 1] >= 0) * (
                                joint_img[:, 1] < img_heights[n])
