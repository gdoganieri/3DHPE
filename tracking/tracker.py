...
class Tracks(object):
    ...
    def __init__(self, detection, trackId, skeleton):
        super(Tracks, self).__init__()
        self.KF = KalmanFilter()
        self.KF.predict()
        self.KF.correct(np.matrix(detection).reshape(3, 1))
...
class Tracker(object):
    ...
    def update(self, skeletons, root_pt):
        detections = skeletons[:, root_pt]
        if len(self.tracks) == 0:
            for i in range(detections.shape[0]):
                track = Tracks(detections[i], self.trackId, skeletons[i])
                self.trackId += 1
                self.tracks.append(track)
        ...
        for i in range(len(detections)):
            if len(self.tracks) > i:
                self.tracks[i].upSkeleton(skeletons[i])
            if i not in assignment:
                track = Tracks(detections[i], self.trackId, skeletons[i])
                self.trackId += 1
                self.tracks.append(track)
